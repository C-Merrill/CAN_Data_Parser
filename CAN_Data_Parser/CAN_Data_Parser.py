...
def send2SQL(timestamp, dataGroup, dataHash, curs):
...
def parse_data(line):
    ...
    send2SQL(myDateTime, re.search(r'function (\w*) at',str(func)).group(1), dataHash, curs)
    return

start = datetime.now()

conn = mysql.connector.connect(user = 'root', password = 'Qxquwn|8pztq9udk', database = "can_data")
curs = conn.cursor()

with zipfile.ZipFile('12-43-24.zip','r') as zipin:
    #with open("output.txt",'w+') as outfile:
    with zipin.open("12-43-24.txt",'r') as infile:
